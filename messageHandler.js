...
const { ESTADOS, ADMIN_NUMBER, PRECO_BASE, ENDERECO, CHAVE_PIX, PADROES, MENSAGEM_AUDIO, MENSAGEM_AUDIO_SUPORTE, MENSAGEM_MULTIPLAS_SEPARADAS, MENSAGEM_ENDERECO_SEM_VIRGULAS, MENSAGEM_STOPACAI, TYPING_DELAY, TAXAS_ENTREGA } = require('../config/constants');
...
class MessageHandler {
  ...
  async processarComprovanteMidia(msg, telefone) {
    ...
    try {
      const resumo = `💰 *COMPROVANTE PIX RECEBIDO*\n` +
        ...
        `📞 *Telefone:* ${Helpers.limparTelefoneParaAdmin(telefone)}\n` +
        `💵 *Valor:* R$ ${stateManager.getDados(telefone, 'valor')}\n` +
        `⏳ Aguardando confirmação (responda "Ok 👍" para aprovar)`;
      
      await this.client.sendMessage(ADMIN_NUMBER, resumo);
      
      // Encaminha a mídia para o admin também
      if (msg.hasMedia) {
        const media = await msg.downloadMedia();
        if (media) {
          await this.client.sendMessage(ADMIN_NUMBER, media, {
            caption: `Comprovante de ${cliente.primeiro_nome}`
          });
        }
      }
    } catch (erro) {
  ...
  async processar(msg, isFromMe = false, isAudio = false, hasMedia = false, mediaType = null) {
    ...
    // T8 - Anti-duplicação
    if (stateManager.isMensagemDuplicada(msg.id._serialized)) {
      Logger.info(`[ANTI-DUP] Mensagem duplicada ignorada: ${msg.id._serialized}`);
      return;
    }
  ...
  async processarStopAcai(msg, telefone) {
    ...
    if (temNomeRegistrado) {
      ...
      try {
        ...
        await this.client.sendMessage(ADMIN_NUMBER, aviso);
      } catch (erro) {
...
Dessa vez vamos montar seu pedido?`;
  ...
  async iniciarAtendimento(msg, telefone, mensagem) {
    ...
    const estadoAbertura = Helpers.dentroDoHorario() ? 'ABERTURA' : 'FORA_HORARIO';
    if (stateManager.jaRespondeuAbertura(telefone, estadoAbertura)) {
      Logger.info(`[I1] Abertura já respondida para ${telefone}, ignorando`);
      return;
    }

    // Verifica horário de funcionamento
    if (!Helpers.dentroDoHorario() && !Helpers.ehModoTeste(mensagem)) {
      // I1 - Marca que respondeu e inicia sessão
      ...
      Logger.log(
        ...
        'Enviada mensagem de fora do horário'
      );
    ...
    if (isClienteAntigo && database.temPedidoAnterior(telefone)) {
    ...
    } else if (isClienteAntigo && clienteExistente.primeiro_nome) {
      // Prompt 7 - Cliente que desistiu (tem nome mas sem pedido anterior)
      // Prompt 7 ALTERADO - Fluxo automático sem aguardar resposta
      const sessao = stateManager.iniciarSessao(telefone, ESTADOS.BUILD_START);
      sessao.dados.clienteNovo = false;
      ...
      await this.enviarMensagemComTyping(msg, 
        `Olá, *${saudacao} ${clienteExistente.primeiro_nome}!* 😍🍧\nQue bom ver você de volta no *Espaço Açaí & Gelatos* 💜\nDessa vez vamos montar seu pedido?`
      );
      ...
      Logger.log(
        ...
        'CLIENTE_DESISTIU',
        'Enviada mensagem de boas-vindas para cliente que desistiu (Prompt 7)'
      );
      
      // Prompt 7 - Continua automaticamente com o cardápio
      ...
      Logger.log(
        ...
        clienteExistente.primeiro_nome,
        'BUILD_START',
        'Cardápio enviado automaticamente para cliente que desistiu'
      );
    } else {
      // Cliente novo
...
Se quiser, pode chamar novamente nesse horário 😍`;
  ...
  async enviarMensagemBoasVindas(msg) {
    const pergunta = `Seja muito bem-vindo(a) ao *Espaço Açaí & Gelatos* 🍧💜
 ...
 na montagem ✍️ Depois de montado e pesado, não pode ser alterado`,
  ...
  async processarFAQSupport(msg, telefone, mensagem) {
    ...
    const primeiraMsg = stateManager.adicionarMensagemSuporte(telefone, mensagem, async (duvidaCompleta) => {
      ...
      try {
        await this.client.sendMessage(ADMIN_NUMBER, resumoSuporteParaAdmin);
...
❯ 3- Zé Gomes (R$ 10,00)`;
  ...
  async processarPrepare(msg, telefone, mensagem) {
    ...
    // E4 - Salva pedido IMEDIATAMENTE no banco
    ...
    resumoAdmin += `📞 *Telefone:* ${Helpers.limparTelefoneParaAdmin(telefone)}\n\n`;
    ...
    // Envia para admin
    try {
      await this.client.sendMessage(ADMIN_NUMBER, resumoAdmin);
  ...
  async enviarMensagemPagamento(chat, telefone) {
    ...
    if (formaEntrega === 'retirada') {
      // Opções simplificadas para RETIRADA
...
*❯ 1-* Pix   *2-* Dinheiro   *3-* Cartão`;
      ...
      if (!stateManager.jaEnviouAlertaPagamento(tel)) {
        try {
          const aviso = `⚠️ *CLIENTE SEM RESPOSTA - FORMA DE PAGAMENTO*\n` +
            ...
            `📞 *Telefone:* ${Helpers.limparTelefoneParaAdmin(tel)}\n` +
  ...
  async enviarMensagemPagamentoPix(msg, telefone) {
    ...
    // Prompt 4 - Mensagem 3
    const msg3 = `*❯* Por favor, envie o comprovante aqui para confirmarmos`;
    ...
    stateManager.iniciarTimerPixComprovante(telefone, async (tel) => {
      ...
      try {
        const aviso = `⚠️ *CLIENTE SEM ENVIAR COMPROVANTE PIX*\n` +
          `👤 *Cliente:* ${cli?.nome_completo || 'Desconhecido'}\n` +
          `📞 *Telefone:* ${Helpers.limparTelefoneParaAdmin(tel)}\n` +
